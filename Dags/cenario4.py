...
"""Cenário 4 (Agrupado): Diferenças nas Propriedades da Coluna (Comprimento):
Descrição: A coluna existe em ambas as tabelas, mas com diferenças nas propriedades (comprimento).
Solução: Modificar o comprimento da coluna na tabela db_clone no banco de dados clonado Db_OfficeClone para se adaptar ao comprimento da coluna correspondente na tabela db_people no banco de dados padrão Db_Office (em casos de redução)."""
...
try:
    # Cenário 4: Modificar o comprimento da coluna na tabela clonada
    for col_name, col_type_cloned in columns_cloned.items():
        ...
        if col_type_main and col_type_cloned and '[]' not in col_type_main and '[]' not in col_type_cloned:
            # Verificar se o comprimento é diferente
            if 'length' in col_type_main and 'length' in col_type_cloned and col_type_main['length'] != col_type_cloned['length']:
                ...
                print(f'Column "{col_name}" length changed from {col_type_cloned["length"]} to {col_type_main["length"]} in {cloned_schema}.{main_table}')

    # Commit apenas se não houve exceções
    transaction.commit()
    print(f"Column length adjusted in {cloned_db}.{cloned_schema}.{main_table}")
